...
const MobAI = () => {
    ...
    const handleSend = async () => {
        ...
        console.log(data, "data");
        settimer(false);
        setchats((chats: any) => [...chats, { id: data.documentId, answer: data.answer.answer, sectionName: data.answer.sectionName, matchScore: data.answer.matchScore }]);
    }
    return (
        <div className='h-[100dvh] overflow-hidden w-full flex pb-16 flex-col bg-gray-50 pt-[60px]'>
                ...
                <div className=' flex flex-col overflow-auto px-3  ml-[5%] w-full flex-1'>
                    <div className='border flex-1 flex flex-col gap-2 p-3'>
                        {loading ? <div className='h-full w-full flex justify-center items-center'>Loading...</div> : chats.length > 0 ? chats.map((chat: any) => {
                                ...
                                <div className='flex justify-start'>
                                    <span className='flex gap-2 items-center'>
                                        <span className='rounded-full w-6 h-6 text-sm font-semibold  text-center border'>
                                            {
                                            chat?.user?.id === user?.id ?  "You"  : chat?.user?.image ? <img key={`chat-img-${chat.id}`} src={chat.user.image} alt="user" className='rounded-full w-full h-full object-cover' /> :
                            ...
                            </div> : <div key={`chat-fallback-${chat.id}`} className={``}>
                                {chat.question ?
                                    ...
                                    <div className='flex items-center gap-2'>
                                        ...
                                        <span className='rounded-full w-6 h-6 text-sm font-semibold  text-center border'>
                                            {
                                                // user?.image ? <img key={`chat-img-${user.id}`} src={user.image} alt="user" className='rounded-full w-full h-full object-cover' /> :
                                    ...
                                    <span className='flex gap-2  justify-end  items-center'>
                                        <div className=' flex flex-col bg-white justify-start text-black w-fit p-2 rounded-md'>
                                            <span className='text-lg font-semibold'>
                                                {chat?.answer}
                                            </span>
                                            <span className='text-sm text-gray-500'>
