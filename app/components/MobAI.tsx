...
//     }
//     return (
//         <div className='h-[100dvh] overflow-hidden w-full flex  flex-col bg-gray-50 pt-[60px]'>
        ...
        <div className='h-[100dvh] overflow-hidden w-full flex  flex-col bg-gray-50 pt-[60px]'>
                ...
                <div className={`w-full h-full  relative  flex flex-col ${sidebar ? "bg-white border-r border-gray-300" : ""}`}>
                    ...
                    <div className={`py-3 flex ${sidebar ? "block" : "hidden"} md:block overflow-auto flex-col gap-3 flex-1`}>
                        {organizations.map((org) => {
                            const currItem = organiz.find(item => item.id === org.id);
                            return (
                                <ul key={`org-${org.id}`} className='bg-white   text-black border  flex  flex-col rounded-md pl-6 pr-2 py-3'>
                                    <li key={`org-li-${org.id}`} className='list-disc px-2 border rounded-md'>
                                        <div className='flex cursor-pointer  justify-between items-center' onClick={() => { setorganiz((orgn) => orgn.map(item => item.id === org.id ? { ...item, open: !item.open } : item)); }}>
                                            <div className=' text-black text-md font-semibold'>
                                                {org.name}
                                            </div>
                                        ...
                                        <div className={`${org.id === currItem?.id && currItem?.open ? "block" : "hidden"}`}>
                                            ...
                                            <div className=' flex gap-3 flex-col'>
                                                {org.projects.map((project) => {
                                                    const currItem = proj.find(item => item.id === project.id);
                                                    return (
                                                        <ul key={`proj-${project.id}`} className='flex py-3 px-2 rounded-md border flex-col'>
                                                            <li key={`proj-li-${project.id}`} className='list-disc'>
                                                                <div className='flex  cursor-pointer justify-between items-center' onClick={() => { setproj(projec => projec.map(proj => proj?.id === project.id ? { ...proj, open: !proj.open } : proj)) }}>
                                                                    <div className='text-black text-sm font-semibold'>
                                                                        {project.name}
                                                                    </div>
                                                            ...
                                                            <div key={`proj-docs-${project.id}`} className={`${project.id === currItem?.id && currItem?.open ? "block" : "hidden"}`}>
                                                                <span className=' text-[12px] text-gray-900'>Documents</span>
                                                                <div className=' flex gap-3 flex-col'>
                                                                    {project.documents.map((document) => {
                                                                        return (
                                                                            <ul key={`doc-${document.id}`} className={`flex py-3 px-6 ${document.status === "processing" ? "hidden" : "block"} rounded-md border flex-col`}>
                                                                                <li key={`doc-li-${document.id}`} className='list-disc'>
                                                                                    <div className='flex cursor-pointer justify-between items-center' onClick={() => { setdoc((docs: any) => { return { id: document.id, open: !docs.open } }); setdocumentId(document.id); setworkspaceId(org.id) }}>
                                                                                        <div className=' text-black text-[12px] font-semibold'>
                                                                                            {document.title}
                                                                                        </div>
            ...
            <div className='flex h-full flex-1 flex-col px-3 border  py-4 '>
                <div className=' flex flex-col overflow-auto px-3  ml-[5%] w-full flex-1'>
                    <div className='border flex-1 flex flex-col gap-2 p-3'>
                        {loading ? <div className='h-full w-full flex justify-center items-center'>Loading...</div> : chats.length > 0 ? chats.map((chat: any) => {
                            return chat.message ? <div key={`chat-${chat.id}`} id={chat.id} className='flex flex-col gap-3'>
                                <div className='flex justify-start'>
                                    <span className='flex gap-2 items-center'>
                                        <span className='rounded-full w-6 h-6 text-sm font-semibold  text-center border'>
                                            {
                                                chat?.user?.image ? <img key={`chat-img-${chat.id}`} src={chat.user.image} alt="user" className='rounded-full w-full h-full object-cover' /> :
                                                    chat?.user?.name ? chat.user.name.slice(0, 2).toUpperCase() : "U"
                                            }
                                        </span>
                                        <div className='bg-black text-white w-fit p-2 rounded-md'>
                            ...
                            </div> : <div key={`chat-fallback-${chat.id}`} className={`${chat.question ? "bg-black border self-start text-white" : "bg-white self-end text-black"} w-fit p-2 rounded-md`}>
                                {chat.question || chat?.answer?.answer}
                            </div>
                        }) : <div key="no-chats" className='h-full w-full flex justify-center items-center'>No chats</div>}
                        <div className='flex justify-end'>
                            <div key="loading-val" className={`bg-white w-[50px] ${timer ? "block" : "hidden"}  px-4  text-2xl text-black`}>
                                {loadingVal}
                            </div>
                        </div>
                    </div>
            ...
            <div className='p-2 flex fixed bottom-0 left-0 w-full  justify-center gap-3 items-center'>
                <input key="chat-input" type="text" ref={inputRef} placeholder='Ask Your knowledge...' className='w-full px-3 outline py-3 bg-white border rounded-lg' />
                <span key="chat-send" className={`bg-blue-500 py-3 ${loading || timer ? "cursor-not-allowed opacity-50" : "cursor-pointer"} text-white  px-5  rounded-md`} onClick={() => { handleSend() }}>send</span>
            </div>
        </div>
    )
}
